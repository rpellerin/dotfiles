...
git-log-by-day () {
  ...
  local LOG_FORMAT=" %Cgreen*%C(dim cyan)%ad -> %cd%C(nodim)%Creset - %s"
  ...
  git log --no-merges --author="${AUTHOR}" --since="${SINCE}" --until="${UNTIL}" --format="%cd" --date=short | sort -u | tac | while read DATE ; do

    local GIT_PAGER=$(git log --format="${LOG_FORMAT}" --since="${DATE} 00:00:00" --until="${DATE} 23:59:59" --author="${AUTHOR}")
