...
do
    ...
    ffmpeg -i "$file" -c copy -metadata:s:v:0 rotate=0 "$dirr/tmp_$b_name" </dev/null
    ffmpeg -i "$dirr/tmp_$b_name" -vf "transpose=2,transpose=2,format=yuv420p" -codec:v libx264 -c:a copy "$dirr/$b_name" </dev/null
    rm "$dirr/tmp_$b_name"
    echo "Next."
done <&0
